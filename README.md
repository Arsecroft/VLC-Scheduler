...
**`recursive: true/false`** — *(optional)* if set to `true`, recurses into subfolders of each directory. Default: `false`.
...
**`media_recursive: true/false`** — *(optional)* sets the default value for recursion of all sources.
